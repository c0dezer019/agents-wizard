...
async function assignAgentToSlot(slotLabel, opts) {
  ...
  const pool = buildSearchIndex(cwd, cwdAgentsDir, cfg).filter(
    (a) => !excludeFiles.includes(a.file),
  );
  ...
  const optionStrings = candidates.map((c) => {
    ...
    const base = `${prefix}${c.name}  [${c.scopeKind}] ${c.label}`;
  ...
  const choice = await pickOption(
    `Assign ${slotLabel}`,
    candidates.length ? [] : ["(no other agents available yet)"],
    options,
    spellSlot(),
  );
  if (choice === null) return null;
...
async function createAgentForSlot(ctx) {
  ...
  const PROJECT_SCOPE = "Project (this project's .claude/agents)";
  const USER_SCOPE = "User (recommended — runnable from any project)";
  const scopeChoice = await pickOption(
    "Create the new agent in which scope?",
    [],
    [USER_SCOPE, PROJECT_SCOPE],
  );
  if (scopeChoice === null) return null;
  const tabKey = scopeChoice === PROJECT_SCOPE ? "project" : "user";
  const { created } = await createFlow(data, tabKey, imageLogo, spellBase64);
