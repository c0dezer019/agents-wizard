...
async function assignAgentToSlot(slotLabel, opts) {
  ...
  for (;;) {
    ...
    const SHOW_ALL = `Show all ${ranked.length} user agent${ranked.length === 1 ? "" : "s"}…`;
    const options = [
      CREATE_NEW,
    ...
    const choice = await pickOption(
      ...
      candidates.length
        ? []
        : [showAll ? "(no other user agents available yet)" : "(no agents read as lead/manager-shaped yet)"],
      options,
    );
    if (choice === null) return null;
