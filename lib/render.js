...
function rowsFor(data, tabKey, projectMode, cfg) {
  ...
  if (tabKey === "user" && data.stackTags && data.stackTags.size > 0) {
    const recommended = [];
    const others = [];
...
function renderList(
  ...
  const viewHint = isBookmarksList || tabKey === "sessions" ? "" : "   v view";
  const copyHint =
    isBookmarksList || tabKey === "sessions" ? "" : "   c copy to project";
  const trackHint = tabKey === "plugin" ? "   u track/untrack → User tab" : "";
  const sessionHint = tabKey === "sessions" ? "   a resume as…" : "";
  const enterLabel = tabKey === "sessions" ? "Enter resume" : "Enter run";
  out +=
    "\n" +
    dim(
      "←/→ tabs   ↑/↓ move   " +
