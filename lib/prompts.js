  ...
  try {
    for (;;) {
      ...
      if (key.ctrl && key.name === "c") process.exit(0);
        ...
        // editInExternalEditor -> openEditor already handles the
        // alt-screen/raw-mode/key-capture pause-resume around spawning the
        // editor — wrapping it again here double-registers the keypress
        // listener (resumeKeyCapture doesn't dedupe), which silently
        // double-fires every keystroke afterward (e.g. one Enter both
        // confirms this field and instantly auto-confirms the next one).
