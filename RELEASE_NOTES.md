...
- 2026-07-13: `--update`'s `git pull` now passes `--tags` too — without it, a tag pushed onto a commit already synced locally (the normal "tag the release after merging" flow) never gets fetched (git only auto-follows tags on newly-downloaded commits), so the tag ref stayed missing locally even after a successful pull.
