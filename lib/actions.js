...
const { rankOrchestratorCandidates } = require("./detect");
const {
  loadTeams,
  saveTeams,
  createTeam,
...
function buildRosterBlock(team, resolvedMembers) {
  ...
  const warnings = omitted.map(
    (r) =>
      `member "${r.ref.name}" ${
        r.status === "missing" ? "is missing" : "was renamed"
      } — omitted from roster.`,
  );
...
async function assignAgentToSlot(slotLabel, opts) {
  const { rankByRole, excludeFiles = [], ctx } = opts;
  const { cwd, cwdAgentsDir, cfg } = ctx;

  // Capped to user-scope agents only — project/plugin agents aren't
  // reliably runnable/delegable the same way across a team's lifetime, so
  // the assignable pool is just ~/.claude/agents (+ tracked plugin links,
  // which buildSearchIndex already reports as scopeKind "user").
  const pool = buildSearchIndex(cwd, cwdAgentsDir, cfg).filter(
    (a) => a.scopeKind === "user" && !excludeFiles.includes(a.file),
  );
  const candidates = rankByRole
    ? rankOrchestratorCandidates(pool)
    : pool.slice().sort((a, b) => a.name.localeCompare(b.name));

  const seen = new Map();
  const optionStrings = candidates.map((c) => {
    const prefix = rankByRole && c.roleScore > 0 ? "★ " : "  ";
    // Descriptions can be multi-paragraph free text — collapse to one line
    // and cap length so a single candidate can't blow out the menu.
    const oneLineDesc = truncate((c.description || "").replace(/\s+/g, " ").trim(), 80);
    const base = `${prefix}${c.name}  — ${oneLineDesc}`;
  ...
  const options = [CREATE_NEW, ...optionStrings];
  const choice = await pickOption(
    `Assign ${slotLabel}`,
    candidates.length ? [] : ["(no other user agents available yet)"],
    options,
  );
  if (choice === null) return null;
  if (choice === CREATE_NEW) return createAgentForSlot(ctx);
  const candidate = candidates[options.indexOf(choice) - 1];
  return candidate ? makeAgentRef(candidate) : null;
}

async function createAgentForSlot(ctx) {
  const { data, imageLogo, spellBase64 } = ctx;
  // Always User scope — the assignable pool is capped to user agents (see
  // assignAgentToSlot), so a project-scope agent created here wouldn't show
  // back up as assignable.
...
async function teamDetailFlow(teamId, ctx) {
  ...
  for (;;) {
    ...
    if (choice === REASSIGN) {
      ...
      const ref = await assignAgentToSlot("orchestrator", { rankByRole: true, excludeFiles, ctx });
      if (ref) {
